...
connect = [1, 1, 2; % first column is element no
           2, 2, 3]; % second and third columns are node numbers 
coord = [1, 0;      %  first column is node number
         2, 0.5;    % second column is x coordinate in m.
         3, 1];
Ip = [0.02, 0, 0.08];   %kg-m^2.

M = zeros(nele+1, nele+1);    % Global mass and stiffness matrices initial values
K = zeros(nele+1, nele+1);

% mass of shaft is neglected
for i=1:nele        
    nd1 = connect(i,2);
    nd2 = connect(i,3);
    x1 = coord(nd1,2);
...
for i=1:nele+1
    wnf(i) = sqrt(1/eig_val(i,i));      
    mode(:,i) = eig_vec(:,i)/max(abs(eig_vec(:,i)));   % Normalised eigen vector
end

for p=1:length(wnf)-1           %keeping natural frequencies in ascending order
  for i =1:length(wnf)-1        %accordingly arranging mode shapes as well 
    if wnf(i)>wnf(i+1)
        temp_wnf = wnf(i);
        wnf(i) = wnf(i+1);
        wnf(i+1) = temp_wnf;
...
%Post processing
disp('Solution is printed to a text file "Assignment4_Q5_224103414.txt"');
Output = fopen('Assignment4_Q5_224103414.txt', 'w');
